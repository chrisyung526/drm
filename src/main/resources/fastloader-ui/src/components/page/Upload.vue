    ...
    export default {
        data () {
            return {
                options: {
                    //目标上传 URL，默认POST
                    target: process.env.VUE_APP_BASE_API+"/client-api/file/chunk",
                    //分块大小(单位：字节)
                    chunkSize: '2048000',
            ...
            */
            onFileSuccess(rootFile, file, response, chunk) {
                ...
                mergeFile(file).then( responseData=> {
                    if(responseData === "FALURE"){
                        alert("上传失败");
                        console.log("合并操作未成功，结果码："+responseData);
                    } else if(responseData === "repeat"){
                        alert("重复上传");
                        console.log("合并操作未成功，结果码："+responseData);
                    }else{
                        console.log(`文件上传完毕 用时：${new Date().getTime() - dtime} ms`);
                        alert(`文件上传完毕 用时：${new Date().getTime() - dtime} ms`);
                    }
