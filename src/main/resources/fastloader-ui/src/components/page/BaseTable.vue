    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="tableData" stripe border class="table" ref="multipleTable" header-cell-class-name="table-header">
                ...
                <el-table-column prop="updateTimeString" label="解析完成时间"></el-table-column>

                <el-table-column label="状态">
                    <template slot-scope="scope">
                        <span v-if="scope.row.fileState === 1">解析中</span>
                        <span v-else-if="scope.row.fileState === 2">解析完成</span>
                    </template>
                </el-table-column>
                <el-table-column prop="resolutionTime" label="解析完成时间(秒)"></el-table-column>
                <el-table-column label="操作" align="center">
                    <template slot-scope="scope">
                        <el-button type="text" icon="el-icon-download" class="blue" @click="handleDownload(scope.$index, scope.row)"
                            >下载</el-button
                        >
                        <el-button type="text" icon="el-icon-delete" class="red" @click="handleDelete(scope.$index, scope.row)"
                            >删除</el-button
                        >
                        <el-button type="text" icon="el-icon-video-play" class="blue" @click="handlePlay(scope.$index, scope.row)"
                            >播放</el-button
                        >
                    </template>
                </el-table-column>
    ...
    methods: {
        ...
        getData() {
            selectFileList(this.query).then((res) => {
                ...
                //this.pageTotal = res.data.total || 100;
            });
        },
        // 触发搜索按钮
        handleSearch() {
        ...
        handlePlay(index, row) {
             this.$router.push({ name: 'play', query: { fileId: row.id } });
